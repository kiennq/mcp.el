...
(defclass mcp-process-connection (jsonrpc-process-connection)
  ((connection-type
   ...
   (-template-resources-callback
    :initarg :template-resources-callback
    :accessor mcp--template-resources-callback))
                                  ...
                                  #'(lambda ()
                                      ...
                                      (when (plist-member capabilities :resources)
                                        ...
                                        (mcp-async-list-resource-templates
                                         connection (mcp--template-resources-callback connection)))))
...
(cl-defun mcp-connect-server (name &key command args url env initial-callback
                                   ...
                                   resources-callback template-resources-callback
                                   error-callback)
    ...
    (when-let* ((server-config (cond (command
      ...
      (let ((connection (apply #'make-instance
                               `(,(pcase connection-type
                                 ...
                                 :template-resources-callback ,template-resources-callback
